...
export const fetchTags = async (limit?: number) => {
    ...
    return request('https://www.forum.effectivealtruism.org/graphql', query).then(({ tags }: { tags: Response<Tag> }) => {
...
export const fetchTag = async (slug: string) => {
    ...
    return request('https://www.forum.effectivealtruism.org/graphql', query).then(({ tag }: { tag: { result: Tag } }) => {
