/**
 * Compiled from:
 * - [CFAR Jargon Appendix](https://forum.effectivealtruism.org/posts/fbv9FWss6ScDMJiAx/appendix-jargon-dictionary).
 * - [LW "Jargon" tag](https://forum.effectivealtruism.org/tag/lesswrong-jargon)
 */

import fs from "fs";
import request, { gql } from "graphql-request";
import { Post } from "../ea/posts";
import { db, fixLinks, getFrontmatter } from "../ea/shared";
...
const loadCAFRJargon = async () => {
    ...
    return request('https://forum.effectivealtruism.org/graphql', query)
...
export const jargonToMD = async () => {
    for (const word of db.jargon) {
        ...
        fs.writeFileSync(`../EA/Jargon/${name}.md`, mdFile)
    }
}
