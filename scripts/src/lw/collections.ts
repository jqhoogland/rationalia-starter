...
export const loadCollections = async (limit?: number) => {
    ...
    return request('https://forum.effectivealtruism.org/graphql', query).then(({ collections }: { collections: Response<Collection> }) => {
...
export const collectionsToMD = async () => {
    for (const collection of db.collections) {
        ...
        fs.writeFileSync(`../EA/Collections/${name}.md`, mdFile)
    }
}
