    ...
    for (const post of db.posts) {
        if (post.forceInclude || post.voteCount > 200) {
            ...
            mdFile += getFrontmatter(
                ...
                {
                    type: "post",
                    tags: ["LessWrong", "Topic", "Post", ...(post?.tags ?? []).map(({name}) => name.replaceAll(" ", "_"))],
                    href: `https://forum.effectivealtruism.org/posts/${post._id}/${post.slug}`,
            ...
            fs.writeFileSync(`../EA/Posts/${name}.md`, mdFile)
        }
    }
}
