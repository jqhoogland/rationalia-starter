...
export const fetchTags = async (limit?: number) => {
    ...
    return request('https://forum.effectivealtruism.org/graphql', query).then(({ tags }: { tags: Response<Tag> }) => {
...
export const fetchTag = async (slug: string) => {
    ...
    return request('https://forum.effectivealtruism.org/graphql', query).then(({ tag }: { tag: { result: Tag } }) => {
...
// (await fetchTags().then(tags => console.log(JSON.stringify(tags, null, 2))))
...
export const tagsToMD = async () => {
    ...
    for (const tag of db.tags.slice(0)) {
        ...
        mdFile += getFrontmatter({
            ...
            href: `https://forum.effectivealtruism.org/tag/${tag.slug}`,
            type: "tag",
            tags: [
                "LessWrong",
                "Topic",
        ...
        fs.writeFileSync(`../EA/Topics/${name}.md`, mdFile)
    }
}
